...
document.getElementById('displayNftsButton').addEventListener('click', async () => {
    ...
    if (results.length > 0) {
        results.forEach(result => {
            ...
            // 如果 metadata 中有 name 属性，可以直接展示，否则使用默认值
            title.textContent = result.metadata.name || 'NFT 卡片';
            nftCard.appendChild(title);

            // 添加 NFT 图片
            if (result.metadata.image) {
                const nftImage = document.createElement('img');
                nftImage.src = result.metadata.image;
                nftImage.alt = result.metadata.name || 'NFT Image';
                nftCard.appendChild(nftImage);
            }

            // 添加 NFT 公钥信息
            const publicKeyInfo = document.createElement('p');
            publicKeyInfo.textContent = `Public Key: ${result.onChainData.publicKey.toString()}`;
            nftCard.appendChild(publicKeyInfo);

            // 你还可以添加更多信息，比如描述、外部链接等
            if (result.metadata.description) {
                const description = document.createElement('p');
                description.textContent = result.metadata.description;
                nftCard.appendChild(description);
            }
    ...
    const isListedArray = listingsArray.map(listings =>
        listings.length > 0 && listings[0].canceledAt === null && listings[0].purchaseReceiptAddress === null
    );
...
document.getElementById('displayNftsButton').addEventListener('click', async () => {
    ...
    if (results.length > 0) {
        results.forEach(result => {
            ...
            // 创建卡片标题（例如可以使用 NFT 名称，如果有的话）
            const title = document.createElement('h3');
            title.textContent = 'NFT 卡片';
            nftCard.appendChild(title);
            ...
            // 你还可以根据需求添加更多的 NFT 详细信息，比如图片、描述等
            // 例如：
            const nftImage = document.createElement('img');
            nftImage.src = result.metadata.imageURL; // 假设 metadata 中有图片 URL
            nftCard.appendChild(nftImage);
